...
import { useGetServiceDetailsQuery, useGetServicesByCategoryQuery } from "@/features/service/serviceApi";
...
const ServicesCarousel = ({ servicesData, servicesLoading, servicesError }: ServicesCarouselProps) => {
  ...
  const { data: serviceDetails, error: serviceDetailsError, isLoading: serviceDetailsLoading } = useGetServiceDetailsQuery(selectedServiceId ? { ServiceID: selectedServiceId } : skipToken);
  ...
  return (
    <div className="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-20 py-6 sm:py-8 md:py-10">
      ...
      <div className="mt-6 sm:mt-8 md:mt-10 flex w-full gap-4 sm:gap-6 flex-col lg:flex-row">
        ...
        <div className="flex-1 w-full lg:max-w-[50%]">
          <h1 className="text-xl sm:text-2xl md:text-3xl mb-3 sm:mb-4 font-semibold">Service Detail</h1>
          <ServiceDetails data={serviceDetails} loading={serviceDetailsLoading} error={serviceDetailsError} />
        </div>
      </div>
    </div>
  );
